...
cat ${tmpFile} | sed -e 's,\s*$,,'   | awk 'BEGIN{lastID="";lastRow=""}{if (lastID!=$1 && cnt[lastID]<1 && lastRow!=""){print lastRow" [EMPTY]";}if ($5!=""){cnt[$1]+=1;print $TAFNet;}lastID=$1;lastRow=$TAFNet}' |sort -k1,1 -k3,3 > ${output}
rm ${tmpFile}
echo `date`
